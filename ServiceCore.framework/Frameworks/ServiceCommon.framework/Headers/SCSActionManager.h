...
/**
 The SCSActionManager class controls the handling and display of the action buttons that are overlaid on the various ServiceSDK interface elements to allow the user to initiate various actions, such as:
 ...
 These built-in actions are managed through the account manager class, and can be customized through the use of it and the associated protocols available.  Furthermore, the SCSActionManagerDelegate protocol can be used to interact with, and customize, the handling of these actions.
...
/**
 ...
 @see setContainerVisible:animated:
 */
 ...
 */
- (BOOL)isActionItemWithNameVisible:(NSString*)actionName;

/**
 Shows or hides an action with the given name.
 
 If the `[SCServiceCloudDelegate serviceCloud:shouldShowActionWithName:]` delegate method is implemented, the result of that method is respected regardless of which value is supplied to `[SCServiceCloud setActionVisible:withName:animated:]`.
 ...
 */
- (void)setActionItemWithName:(NSString*)name visible:(BOOL)visible animated:(BOOL)animated;
 ...
 */
- (void)performActionWithName:(NSString*)actionName actionItem:(nullable UIView<SCSActionItem>*)actionItem;
